...
@section('titulo')
    Registrate en Laravel - tailwind
@endsection
    ...
    <div class="md:flex md:justify-center md:gap-10 md:items-center">
        ...
        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
            <form action="{{ route('register')}} " method="POST">
                @csrf
                <div calss="mt-10 md-5">
                    <label id="name" class="md-2 block uppercase text-gray-500 font-bold">
                        Nombre
                    </label>
                ...
                <div calss="md-5">
                    <label id="email" class="md-2 block uppercase text-gray-500 font-bold">
                        Email
                    </label>
                    <input type="text" id="email" name="email" placeholder="Tu Email" class="border p-3 w-full rounded-lg  @error('email') border-red-500 @enderror" value="{{ old('email') }}">
                    @error('email')
                        <p class="bg-red-500 text-white my-2 rounded-t-lg text-sm p-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div calss="md-5">
                    <label id="password" class="md-2 block uppercase password-gray-500 font-bold">
                        Repetir Password
                    </label>
                ...
                <input type="submit" value="Crear Cuenta" class="bg-sky-600 hover:bg-sky-700 transition-colors cursor-pointer uppercase font-blod w-full p-3 text-white rounded-lg">
            </form>
        </div>
    </div>
